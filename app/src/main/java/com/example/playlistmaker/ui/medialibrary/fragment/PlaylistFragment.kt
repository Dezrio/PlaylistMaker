...
import com.example.playlistmaker.ui.medialibrary.adapter.PlaylistTrackAdapter
import com.example.playlistmaker.ui.medialibrary.models.PlaylistInfo
import com.example.playlistmaker.ui.medialibrary.view_model.PlaylistScreenState
...
class PlaylistFragment : BindingFragment<FragmentPlaylistBinding>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.tvEditPlaylist.setOnClickListener {
            findNavController().navigate(
                R.id.action_playlistFragment_to_addPlaylistFragment,
                AddPlaylistFragment.setArgs(playlistId)
            )
        }
    ...
    private fun renderScreenState(state: PlaylistScreenState) {
        when (state) {
            ...
            is PlaylistScreenState.Found -> showContent(state.info)
        }
    }
    ...
    private fun showContent(info: PlaylistInfo) {
        setPlaylistDetails(info)
    ...
    private fun setPlaylistDetails(info: PlaylistInfo) {
        ...
        if (info.description.isNotBlank()) {
            binding.tvPlaylistDescription.text = info.description
        ...
        binding.tvPlaylistSumDuration.text = info.tracksDuration

        val trackCountString = info.tracksCountString
        binding.tvPlaylistTracksCount.text = trackCountString

        trackAdapter.updateTracks(info.tracks)
        binding.rvTracks.isVisible = info.tracks.isNotEmpty()
    ...
    private fun openPlayer(track: Track) {
        val action =
            PlaylistFragmentDirections.actionPlaylistFragmentToAudioPlayerActivity(track)
    ...
    private fun calculateBottomSheetTracksHeight(): Int {
        ...
        val bottomSheetTracksIndentationPx =
            requireActivity().resources.getDimension(R.dimen.sheet_tracks_size)
                .toInt()
